                    ...
                    <div className="container mx-auto flex flex-wrap items-center justify-between">
                        ...
                        <h1 className="flex justify-center items-center text-5xl w-1/3">
                            ...
                            <ul className="flex justify-end w-1/3 pr-2">
                                {jwt === null || jwt === "" ? 
                                    <li key="login" className="px-2 py-2 text-lg cursor-pointer hover:bg-gray-50 rounded-md"><Link className="flex flex-col " to={"/login"}><FaRegUserCircle className="block mx-auto" size={25}/><p className="text-sm">Login</p></Link></li>
                                    :
                                    <div>
                                        ...
                                        <div className="hidden peer-hover:flex hover:flex flex-col bg-white drop-shadow-lg cursor-pointer">
                                            <ul key="profile" className="bg-white absolute py-2 px-2 w-full text-center hover:bg-gray-50 cursor-pointer" onClick={toProfile}>Profile</ul>
                                            <ul key="orders" className="bg-white absolute mt-10 py-2 px-2 w-full text-center hover:bg-gray-50 cursor-pointer" onClick={toOrders}>Orders</ul>
                                            <ul key="signout" className="bg-white absolute mt-20 py-2  px-2 w-full text-center hover:bg-red-300 cursor-pointer" onClick={signOut}>Log Out</ul>
                                        </div>
                                    </div>
                                }
                                <li key="cart" className="py-2 text-lg rounded-md hover:bg-gray-50"><Link className="flex py-1 px-2" to={"/cart"}><CgShoppingCart size={30}/> {cartQuantity}</Link></li>
                            </ul>
                    </div>
                </nav>
